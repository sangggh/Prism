  ...
  return (
    <main className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-4xl mx-auto py-16 px-8">
        ...
        >
          <h1 className="text-4xl font-black text-slate-900 tracking-tight mb-4">Your Career Roadmap</h1>
            ...
            return (
              ...
              >
                ...
                {index !== ROADMAP_STEPS.length - 1 && (
                  <div className="absolute top-16 left-8 w-0.5 h-full bg-slate-200" />
                )}

                <div className={`w-16 h-16 rounded-2xl flex-shrink-0 flex items-center justify-center shadow-lg shadow-slate-100 ${step.color}`}>
                  <Icon size={32} />
                </div>

                <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm group-hover:shadow-xl group-hover:shadow-slate-200/50 transition-all flex-1">
                  <h3 className="text-xl font-bold text-slate-900 mb-2">{step.title}</h3>
                  <p className="text-slate-500 mb-6 leading-relaxed">{step.description}</p>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {step.items.map((item) => (
                      <div key={item} className="flex items-center gap-2 text-sm font-semibold text-slate-700 bg-slate-50 px-4 py-2 rounded-xl">
